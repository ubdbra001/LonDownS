...
eventsToFind   = eventMarkers(eventInd_t,:);                               % Put these markers into a variable
...
if ~isempty(eventsToFind)                                                  % Check for user input
    for folder_n = 1:size(folders,1)                                       % Loop through each participant
        ...
        allEvents(:,3) = cellfun(@(x) x{1},...
            eventBuffer(:,3),'UniformOutput', false);                      % Add the trigger label from eventBuffer to the last column of allData
        ...
        clearvars(varsToClear{:}, '-except', 'folder_n')        
    end % folder_n
end
